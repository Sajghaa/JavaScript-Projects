// CalendarView.js - Renders calendar view
class CalendarView {
    ...
    render() {
        ...
        let html = `
            <div class="calendar-header">
                <button id="calendarPrevBtn" class="btn btn-secondary">
                    <i class="fas fa-chevron-left"></i>
                </button>
                <h2>${monthNames[month]} ${year}</h2>
                <button id="calendarNextBtn" class="btn btn-secondary">
    ...
    attachEvents() {
        ...
        // Day click handlers (optional - can add new task on day click)
        const calendarDays = document.querySelectorAll('.calendar-day:not(.empty)');
        calendarDays.forEach(dayEl => {
            dayEl.addEventListener('dblclick', () => {
                const date = dayEl.dataset.date;
                if (date && this.taskManager) {
                    // Pre-fill due date in add task modal
